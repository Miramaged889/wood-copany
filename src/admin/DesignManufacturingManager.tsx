...
import { DesignManufacturingItem } from "./dataService";

interface DesignManufacturingManagerProps {
  data: DesignManufacturingItem[];
  categories: string[];
  onSave: (data: DesignManufacturingItem[]) => void;
}

const DesignManufacturingManager: React.FC<DesignManufacturingManagerProps> = ({
  data,
  categories,
  onSave,
}) => {
  const [items, setItems] = useState<DesignManufacturingItem[]>(data);
  const [showForm, setShowForm] = useState(false);
  const [editingItem, setEditingItem] = useState<DesignManufacturingItem | null>(null);
  const [selectedItem, setSelectedItem] = useState<DesignManufacturingItem | null>(null);
  ...
  const handleEdit = (item: DesignManufacturingItem) => {
    setEditingItem(item);
    setFormData({
      title: item.title,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingItem) {
      ...
      // Add new item
      const newItem: DesignManufacturingItem = {
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex space-x-4 space-x-reverse pt-4">
                  ...
                  >
                    ...
                    <span>
                      {editingItem ? "حفظ التعديلات" : "إضافة المشروع"}
                    </span>
                  </button>
                  <button
                    type="button"
      ...
      {selectedItem && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-2xl w-full">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold">تفاصيل المشروع</h3>
                <button
...
export default DesignManufacturingManager;
